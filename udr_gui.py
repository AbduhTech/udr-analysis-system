...
class UDRGUI:
    ...
    def get_int(self, entry, name):
        try:
            ...
            messagebox.showerror(
                "Input error", f"'{name}' must be a whole number.")
            raise
    ...
    def build_observation_tab(self):
        ...
        self.obs_students = self.labelled_entry(
            form, "Total students in cohort",          0, "20", "thesis value: 20")
        self.obs_sessions = self.labelled_entry(
            form, "Number of sessions",                1, "7",  "thesis value: 7")
        self.obs_ai_first = self.labelled_entry(
            form, "Students using AI as first resort", 2, "18", "observed in sessions")
        self.obs_unable = self.labelled_entry(
            form, "Students unable to explain code",   3, "16", "verbal response check")
        self.obs_debug = self.labelled_entry(
            form, "Students with Debugging weakness",  4, "17", "UDR domain — D")
        self.obs_reason = self.labelled_entry(
            form, "Students with Reasoning weakness",  5, "16", "UDR domain — R")
        self.obs_passed = self.labelled_entry(
            form, "Students passed despite deficits",  6, "20", "institutional gap")
        self.obs_drop = self.labelled_entry(
            form, "Students with motivation decline",  7, "12", "abandoned take-home")
    ...
    def analyze_observation(self):
        ...
        self.observation_results = analyze_observation(
            s, ses, ai, un, db, re, pa, dr)
        r = self.observation_results

        self.obs_box.delete("1.0", tk.END)
        self.obs_box.insert(
            tk.END, "OBSERVATION MODULE RESULTS — Formula F1: P(i) = (f(i)/n) × 100\n")
        self.obs_box.insert(tk.END, "─" * 65 + "\n")
        self.obs_box.insert(
            tk.END, f"Students observed:          {r['students']}\n")
        self.obs_box.insert(
            tk.END, f"Sessions conducted:         {r['sessions']}\n\n")
        self.obs_box.insert(tk.END, f"UDR DOMAIN RATES (F1):\n")
        self.obs_box.insert(
            tk.END, f"  AI dependency rate:       {r['ai_dependency_rate']}%\n")
        self.obs_box.insert(
            tk.END, f"  Explanation failure rate: {r['explanation_failure_rate']}%  ← Understanding\n")
        self.obs_box.insert(
            tk.END, f"  Debugging weakness rate:  {r['debugging_weakness_rate']}%  ← Debugging (RQ2)\n")
        self.obs_box.insert(
            tk.END, f"  Reasoning weakness rate:  {r['reasoning_weakness_rate']}%  ← Reasoning\n\n")
        self.obs_box.insert(tk.END, f"INSTITUTIONAL GAP:\n")
        self.obs_box.insert(
            tk.END, f"  Institutional gap rate:   {r['institutional_gap_rate']}%\n")
        self.obs_box.insert(
            tk.END, f"  Motivation decline rate:  {r['attendance_decline_rate']}%\n\n")
    ...
    def build_survey_tab(self):
        ...
        ttk.Entry(top, textvariable=self.csv_path,
                  width=65).grid(row=0, column=0, padx=8)
        ttk.Button(top, text="Browse CSV", command=self.browse_csv).grid(
    ...
    def analyze_survey(self):
        ...
        if path and os.path.exists(path):
            try:
                ...
                self.survey_box.insert(
                    tk.END, f"CSV loaded: {os.path.basename(path)}\n")
            ...
            self.survey_box.insert(
                tk.END, "No CSV selected — using verified thesis values.\n")
        ...
        self.survey_box.insert(
            tk.END, f"  Daily AI use:         {r['freq'].get('daily', 0)}/{n} = {r['daily_pct']}%\n")
        self.survey_box.insert(
            tk.END, f"  Weekly AI use:        {r['freq'].get('weekly', 0)}/{n} = {r['weekly_pct']}%\n")
        self.survey_box.insert(
            tk.END, f"  Daily+Weekly (RQ1):   {r['daily_weekly_pct']}%\n\n")
        self.survey_box.insert(tk.END, "F2 — LIKERT AGREEMENT SCORES:\n")
        self.survey_box.insert(
            tk.END, f"  Q10 Explain concepts (U):  {r['q10_n']}/{n} = {r['q10_pct']}%\n")
        self.survey_box.insert(
            tk.END, f"  Q11 Write without AI (U):  {r['q11_n']}/{n} = {r['q11_pct']}%\n")
        self.survey_box.insert(
            tk.END, f"  Q16 Debug without AI (D):  {r['q16_n']}/{n} = {r['q16_pct']}%\n")
        self.survey_box.insert(
            tk.END, f"  Q17 AI-first Likert (D):   {r['q17_n']}/{n} = {r['q17_pct']}%\n")
        self.survey_box.insert(
            tk.END, f"  Q18 Trace execution (R):   {r['q18_n']}/{n} = {r['q18_pct']}%\n")
        self.survey_box.insert(
            tk.END, f"  Q19 Redo without AI (R):   {r['q19_n']}/{n} = {r['q19_pct']}%\n")
        self.survey_box.insert(
            tk.END, f"  Q24 Passing w/o learning:  {r['q24_n']}/{n} = {r['q24_pct']}%\n\n")
        self.survey_box.insert(tk.END, "F2b — LIKERT DISAGREEMENT SCORE:\n")
        self.survey_box.insert(
            tk.END, f"  Q32 No AI training:        {r['q32_n']}/{n} = {r['q32_pct']}%\n\n")
        self.survey_box.insert(
            tk.END, "F3 — COMPETENCE PERCEPTION GAP (RQ3):\n")
        self.survey_box.insert(
            tk.END, f"  G_competence = {r['q10_pct']}% - {r['q11_pct']}% = {r['g_competence']} pts\n\n")
        self.survey_box.insert(
            tk.END, "F4 — SOCIAL DESIRABILITY GAP (RQ2, RQ4):\n")
        self.survey_box.insert(
            tk.END, f"  Q14 AI-first (direct):     {r['q14_ai_n']}/{n} = {r['q14_ai_pct']}%\n")
        self.survey_box.insert(
            tk.END, f"  G_SDB = {r['q17_pct']}% - {r['q14_ai_pct']}% = {r['g_sdb']} pts\n\n")
        self.survey_box.insert(tk.END, "F5 — INSTITUTIONAL GAP (Gap 5):\n")
        self.survey_box.insert(
            tk.END, f"  Supply gap D(Q32):         {r['institutional_supply']}%\n")
        self.survey_box.insert(
            tk.END, f"  Demand A(Q34):             {r['institutional_demand']}%\n")

    def _get_all_data(self):
        """Return (obs, surv, case, final) — using demo values for any missing component."""
        from modules.observation_module import analyze_observation_demo
        from modules.survey_module import analyze_survey_demo
        from modules.case_study_module import analyze_case_study_demo
        # Re-use cached triangulation result if already computed, otherwise compute fresh
        if hasattr(self, '_last_final'):
            return self._last_final
        obs = self.observation_results or analyze_observation_demo()
        surv = self.survey_results or analyze_survey_demo()
        case = self.case_results or analyze_case_study_demo()
        final = triangulate(obs, surv, case)
        return obs, surv, case, final

    def generate_figures(self):
        if not self.survey_results:
            self.analyze_survey()
        try:
            obs, surv, case, final = self._get_all_data()
            paths = generate_all_figures(surv, obs, case, final)
            messagebox.showinfo("Figures saved",
                                f"{len(paths)} figures generated:\n" + "\n".join(paths))
        except Exception as e:
    ...
    def build_case_tab(self):
        ...
        # ── Action buttons — placed at the top so they are always visible ──
        case_btn_frame = tk.Frame(self.case_tab)
        case_btn_frame.pack(pady=10)
        ttk.Button(case_btn_frame, text="Analyze Case Study (F7)",
                   command=self.analyze_case).pack(side="left", padx=8, ipadx=10, ipady=4)
        ttk.Button(case_btn_frame, text="View Component 3 Results",
                   command=lambda: [self.view_component3(),
                                    self.tabs.select(self.results_tab)]).pack(side="left", padx=8, ipadx=10, ipady=4)
        ttk.Button(case_btn_frame, text="Go to Integrated Results (F6)",
                   command=lambda: self.tabs.select(self.results_tab)).pack(side="left", padx=8, ipadx=10, ipady=4)
        ...
        tk.Label(frame, text="\nTask A — WITHOUT AI (isPrime program)",
                 font=("Arial", 11, "bold")).grid(row=2, column=0, columnspan=3,
                                                  pady=6, sticky="w", padx=10)
        self.ta_u = self.labelled_entry(
            frame, "Understanding indicators present (0–4)", 3, "0", "F7 — Task A")
        self.ta_d = self.labelled_entry(
            frame, "Debugging indicators present (0–4)",    4, "0", "F7 — Task A")
        self.ta_r = self.labelled_entry(
            frame, "Reasoning indicators present (0–4)",    5, "0", "F7 — Task A")

        tk.Label(frame, text="\nTask B — WITH AI (string-reversal bugfix)",
                 font=("Arial", 11, "bold")).grid(row=6, column=0, columnspan=3,
                                                  pady=6, sticky="w", padx=10)
        self.tb_u = self.labelled_entry(
            frame, "Understanding indicators present (0–4)", 7, "0", "F7 — Task B")
        self.tb_d = self.labelled_entry(
            frame, "Debugging indicators present (0–4)",    8, "0", "F7 — Task B")
        self.tb_r = self.labelled_entry(
            frame, "Reasoning indicators present (0–4)",    9, "1", "F7 — Task B (1 partial)")

        tk.Label(frame, text="\nBehavioral observations",
                 font=("Arial", 11, "bold")).grid(row=10, column=0, columnspan=3,
                                                  pady=6, sticky="w", padx=10)
        self.case_dep = self.labelled_entry(
            frame, "AI dependency level (1–10)",        11, "9",  "9 = very high")

        tk.Label(frame, text="Used AI before thinking independently?").grid(
            row=12, column=0, sticky="w", padx=10, pady=5)
        self.case_ai_before = ttk.Combobox(
            frame, values=["Yes", "No"], width=18)
    ...
    def analyze_case(self):
        ...
        self.case_box.insert(
            tk.END, "CASE STUDY MODULE RESULTS — Formula F7: UDR_score = (indicators/4) × 100\n")
        self.case_box.insert(tk.END, "─" * 65 + "\n")
        self.case_box.insert(
            tk.END, f"Claimed self-reliance (interview): {claimed}%\n\n")
        self.case_box.insert(tk.END, f"TASK A (without AI):\n")
        self.case_box.insert(
            tk.END, f"  Understanding: {ta_u}/4 indicators = {r['task_a_understanding']}%\n")
        self.case_box.insert(
            tk.END, f"  Debugging:     {ta_d}/4 indicators = {r['task_a_debugging']}%\n")
        self.case_box.insert(
            tk.END, f"  Reasoning:     {ta_r}/4 indicators = {r['task_a_reasoning']}%\n")
        self.case_box.insert(
            tk.END, f"  Average UDR:   {r['task_a_average']}%\n\n")
        self.case_box.insert(tk.END, f"TASK B (with AI):\n")
        self.case_box.insert(
            tk.END, f"  Understanding: {tb_u}/4 indicators = {r['task_b_understanding']}%\n")
        self.case_box.insert(
            tk.END, f"  Debugging:     {tb_d}/4 indicators = {r['task_b_debugging']}%\n")
        self.case_box.insert(
            tk.END, f"  Reasoning:     {tb_r}/4 indicators = {r['task_b_reasoning']}%\n")
        self.case_box.insert(
            tk.END, f"  Average UDR:   {r['task_b_average']}%\n\n")
        self.case_box.insert(tk.END, f"ILLUSORY COMPETENCE GAP (F7 + F3):\n")
        self.case_box.insert(
            tk.END, f"  Claimed: {claimed}%  |  Observed (Task A avg): {r['task_a_average']}%\n")
        self.case_box.insert(
            tk.END, f"  Gap: {r['illusory_gap']} pts — {r['illusory_competence']}\n")
        self.case_box.insert(
            tk.END, f"  Classification: {r['classification']}\n\n")
        ...
        self.case_box.insert(tk.END,
                             "\n─────────────────────────────────────────────────────────────\n"
                             "✓  Component 3 analysis complete.\n"
                             "   Switch to the 'Integrated Results' tab to run F6 triangulation.\n")
        self.case_box.see("1.0")
        self.tabs.select(self.case_tab)
    ...
    def view_component3(self):
        self.results_box.delete("1.0", tk.END)
        if not self.case_results:
            self.results_box.insert(tk.END,
                                    "Component 3 has not been run yet. Go to the Component 3 tab, "
                                    "enter indicator counts, and click Analyze Case Study (F7) first.")
            return
        r = self.case_results
        self.results_box.insert(
            tk.END, "COMPONENT 3 — CASE STUDY FULL RESULTS\n")
        self.results_box.insert(tk.END, "=" * 70 + "\n\n")
        self.results_box.insert(tk.END, "TASK A (without AI):\n")
        self.results_box.insert(
            tk.END, f"  Understanding: {r['task_a_understanding']}%\n")
        self.results_box.insert(
            tk.END, f"  Debugging:     {r['task_a_debugging']}%\n")
        self.results_box.insert(
            tk.END, f"  Reasoning:     {r['task_a_reasoning']}%\n")
        self.results_box.insert(
            tk.END, f"  Average UDR:   {r['task_a_average']}%\n\n")
        self.results_box.insert(tk.END, "TASK B (with AI):\n")
        self.results_box.insert(
            tk.END, f"  Understanding: {r['task_b_understanding']}%\n")
        self.results_box.insert(
            tk.END, f"  Debugging:     {r['task_b_debugging']}%\n")
        self.results_box.insert(
            tk.END, f"  Reasoning:     {r['task_b_reasoning']}%\n")
        self.results_box.insert(
            tk.END, f"  Average UDR:   {r['task_b_average']}%\n\n")
        self.results_box.insert(tk.END, "ILLUSORY COMPETENCE GAP:\n")
        self.results_box.insert(
            tk.END, f"  Illusory gap:        {r['illusory_gap']} pts\n")
        self.results_box.insert(
            tk.END, f"  Illusory competence: {r['illusory_competence']}\n")
        self.results_box.insert(
            tk.END, f"  Classification:      {r['classification']}\n")
        self.results_box.insert(
            tk.END, f"  Claimed confidence:  {r['claimed_confidence']}%\n\n")
        self.results_box.insert(tk.END, "RQ ANSWERS FROM COMPONENT 3:\n")
        self.results_box.insert(tk.END, f"  {r['rq1_answer']}\n")
        self.results_box.insert(tk.END, f"  {r['rq2_answer']}\n")
        self.results_box.insert(tk.END, f"  {r['rq3_answer']}\n")
        self.results_box.insert(tk.END, f"  {r['rq4_answer']}\n")
    ...
    def final_results(self):
        ...
        # Compact Component 3 summary
        self.results_box.insert(
            tk.END, "COMPONENT 3 SUMMARY — Case Study (Student A)\n")
        self.results_box.insert(tk.END, "─" * 70 + "\n")
        self.results_box.insert(tk.END,
                                f"  Task A (without AI):  U={case['task_a_understanding']}%  "
                                f"D={case['task_a_debugging']}%  R={case['task_a_reasoning']}%  "
                                f"Avg={case['task_a_average']}%\n"
                                f"  Task B (with AI):     U={case['task_b_understanding']}%  "
                                f"D={case['task_b_debugging']}%  R={case['task_b_reasoning']}%  "
                                f"Avg={case['task_b_average']}%\n"
                                f"  Illusory gap:         {case['illusory_gap']} pts  |  "
                                f"Classification: {case['classification']}\n"
                                f"  Click 'View Component 3 Results' for the full breakdown.\n\n")

        # F6 triangulation status
        self.results_box.insert(
            tk.END, "FORMULA F6 — TRIANGULATION VALIDITY\n")
        self.results_box.insert(
            tk.END, "Finding is valid when: C1 AND C2 AND C3 all support it\n\n")
        findings = [
            ('Finding 1: AI dependency as default',
             final['finding1_ai_dependency_valid']),
            ('Finding 2: Illusory competence gap',
             final['finding2_illusory_competence_valid']),
            ('Finding 3: Debugging most affected',
             final['finding3_debugging_affected_valid']),
            ('Finding 4: Dual institutional failure',
             final['finding4_institutional_failure_valid']),
        ]
        ...
        self.results_box.insert(tk.END, "\n" + "=" * 70 + "\n")
        self.results_box.insert(
            tk.END, "RESEARCH QUESTIONS — ANSWERS FROM SYSTEM OUTPUT\n")
        self.results_box.insert(tk.END, "=" * 70 + "\n\n")
    ...
    def save_report(self):
        ...
        messagebox.showinfo(
            "Saved", "Report saved to outputs/results_report.txt")
